...
# Ensure an asyncio event loop exists on the main thread before creating the client.
# Telethon expects an event loop to be available; on some Python versions there is
# no running loop until later, which causes a RuntimeError. Create and set one
# if needed so the client can be constructed at module import time.
try:
    # If a running loop exists this will succeed; otherwise it raises RuntimeError
    asyncio.get_running_loop()
except RuntimeError:
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

# Now safe to construct the TelegramClient
...
async def decrypt_handler(event):
    ...
    # Create buttons for each pending image and include message id + timestamp in the label
    buttons = []
    for image in pending_images:
        sender_name = image.get('sender_name', 'Unknown')
        message_id = image.get('message_id')
        ts = image.get('timestamp')
        # Try to parse ISO timestamp and format it nicely; fall back to raw string
        try:
            nice_ts = datetime.fromisoformat(ts).strftime('%Y-%m-%d %H:%M')
        except Exception:
            nice_ts = ts

        # Shorten the message id for display to keep button labels concise
        short_id = message_id[:8] if message_id else 'unknown'
        label = f"{sender_name} • ID:{short_id} • {nice_ts}"
        buttons.append([Button.inline(label, data=f"decrypt_{message_id}")])
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    try:
                        # First try to fetch a scrambled preview image (JPEG) so the recipient can *see* an encrypted-looking image
                        try:
                            preview_sent = False
                            async with aiohttp.ClientSession() as dl_session:
                                async with dl_session.get(f"{API_BASE_URL}/preview/{message_id}") as dl_resp:
                                    if dl_resp.status == 200:
                                        # Save to a temporary file and send it as an image
                                        with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as img_tmp:
                                            img_path = img_tmp.name
                                            img_tmp.write(await dl_resp.read())

                                        await bot.send_file(
                                            recipient_id,
                                            img_path,
                                            caption=(f"You have received an encrypted image from user {user_id}.\n"
                            ...
                            if not preview_sent:
                                # Fall back: try to send the raw encrypted binary if preview unavailable
                                async with aiohttp.ClientSession() as dl_session:
                                    async with dl_session.get(f"{API_BASE_URL}/download_encrypted/{message_id}") as dl_resp:
                                        if dl_resp.status == 200:
                                            with tempfile.NamedTemporaryFile(suffix='.bin', delete=False) as enc_tmp:
                                                enc_path = enc_tmp.name
                                                enc_tmp.write(await dl_resp.read())

                                            await bot.send_file(
                                                recipient_id,
                                                enc_path,
                                                caption=(f"You have received an encrypted file from user {user_id}.\n"
                                                         f"Message ID: {message_id}\n"
                                                         "Use /decrypt to view and decrypt this image.")
                                            )
                                            try:
                                            ...
                                            # If download failed, fall back to a notification message
                            ...
                            logger.error(f"Failed to forward encrypted content to recipient: {e}")
                            await bot.send_message(
                                recipient_id,
                                f"You have received an encrypted image from user {user_id}.\n"
                                f"Message ID: {message_id}\n"
